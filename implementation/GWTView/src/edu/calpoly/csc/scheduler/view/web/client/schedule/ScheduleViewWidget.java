...
public class ScheduleViewWidget implements CloseHandler<PopupPanel> {
	...
	/**
	 * Resets days Mon, Tue, Wed, Thu, Fri to align with columns 1, 2, 3, 4, and
	 * 5
	 */
	...
	/**
	 * Places the times 7am-10pm at half hour increments as the rows in the
	 * first column in the schedule.
	 */
	private void setTimes() {
		...
		for (i = 0; i < times.length; i++) {
			scheduleGrid.setWidget(i + 1, 0,
					new HTML("<b>" + times[i] + "</b>"));
		}
	}
			...
			for (ScheduleItemGWT item : scheduleItems) {
				if (item.isPlaced()) {
					...
					for (int occDay : occupiedDays) {
						/*
						 * If the item placed item occupies times at the same
						 * time and same day then increment the number of
						 * overlaps at the examined time.
						 */
	...
	private void expandDay(int day) {
		...
		for (i = 1; i <= numberOfTimeSlots; i++) {
			...
			for (j = day; j <= numberOfDays; j++) {
				columnsOfDays.get(i - 1).set(j,
						columnsOfDays.get(i - 1).get(j) + 1);
			}
		}
	}
	...
	private void placeScheduleItem(ScheduleItemGWT placedSchdItem,
		...
		for (int dayNum : schdItemDays) {
			...
			if (!filteredDays.contains(dayNum)) {
				/*
				 * Get the maximum number of times this item will overlap with
				 * already placed items
				 */
				overlapCount = overlaps(placedSchdItem, dayNum);
				/*
				 * If the overlap count is larger than the span of the day
				 * column, expand the column
				 */
				if (overlapCount >= dayColumnSpans.get(dayNum - 1)) {
					expandDay(dayNum);
				}
				// Get the column which this day aligns with
				dayCol = columnsOfDays.get(rowRangeStart - 1).get(dayNum - 1)
						.intValue();
				...
				((SimplePanel) scheduleGrid.getWidget(rowRangeStart, dayCol))
						.setWidget(schdItem);
				// Set the schedule item to span rows which its time occupies
				scheduleGrid.getFlexCellFormatter().setRowSpan(rowRangeStart,
						dayCol, rowRangeEnd - rowRangeStart);
				...
				for (j = rowRangeStart; j < rowRangeEnd - 1; j++) {
					...
					scheduleGrid.removeCell(j + 1,
							(scheduleGrid.getCellCount(j + 1) - 1));
				}
			}
		}
