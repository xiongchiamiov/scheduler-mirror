				...
				atHalfHour, inSchedule, schedItems, new AsyncCallback<ScheduleItemList>() {
					...
					public void onSuccess(ScheduleItemList rescheduled) {
						...
						if (!inSchedule) {
							...
							sectionsIncluded = includedListBox
									.getSectionsInBox(courseHolder);
							itemIndex = includedListBox
									.contains(new CourseListItem(courseHolder,
											true));
							...
							if (itemIndex >= 0) {
								...
								if (fromIncluded
										|| sectionsIncluded
												+ getSectionsOnSchedule(courseHolder) == availableListBox
												.getSectionsInBox(courseHolder)) {
									...
									// section
										...
										includedListBox.remove(includedListBox
												.getWidget(itemIndex));
									}
								}
							}
