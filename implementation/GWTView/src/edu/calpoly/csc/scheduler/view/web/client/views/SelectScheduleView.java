...
public class SelectScheduleView extends VerticalPanel implements IViewContents, OtherFilesStrategy
{
   ...
   public SelectScheduleView(final GreetingServiceAsync service, final MenuBar menuBar, final String username)
   {
      ...
      FlowPanel botflow = new FlowPanel();
   ...
   public void afterPush(ViewFrame frame)
   {
      ...
      service.getScheduleNames(new AsyncCallback<Map<String, UserDataGWT>>()
      {
         ...
         public void onSuccess(Map<String, UserDataGWT> result)
         {
            ...
            {
               addNewDocument(scheduleName, availableSchedulesByName.get(scheduleName).getScheduleID());
   ...
   {
      if (!name.startsWith(ScheduleTrashView.TRASH_PREFIX))
      {
...
//         doc.setWidth("100%");
         ...
         doc.add(cb);
         doc.setCellHorizontalAlignment(cb, ALIGN_LEFT);
   ...
   protected void openLoadedSchedule(Integer scheduleID, String scheduleName)
   {
      ...
      if (myFrame.canPopViewsAboveMe())
      {
         ...
         myFrame.frameViewAndPushAboveMe(new AdminScheduleNavView(service, this, menuBar, username, scheduleID,
               scheduleName));
