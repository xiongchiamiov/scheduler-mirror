				...
				if (weekAvailBuf != null && coursePrefsBuf != null) {
					try {
						...
						objectIn = new ObjectInputStream(
								new ByteArrayInputStream(weekAvailBuf));
						toAdd.setAvailability((WeekAvail) objectIn.readObject());
						objectIn = new ObjectInputStream(
								new ByteArrayInputStream(coursePrefsBuf));
...
//								.readObject());
	 ...
	 * 
	 * @return Instructors that can teach this class without going over their
	 *         WTU limit, in order of most wanting to teach the class first
	 */
	public Vector<Instructor> getInstructorsForCourse(final Course course) {
		...
		Collections.<Instructor> sort(instructors,
				new Comparator<Instructor>() {
					@Override
					public int compare(Instructor o1, Instructor o2) {
						return o1.getPreference(course)
								- o2.getPreference(course);
					}
				});
		return instructors;
	}
