   ...
   public Course getLab(ArrayList<Course> arrList, int lab) {
	...
	public CourseDB getCourseDB() {
		...
		try {
			...
			sql = sql + "labPairing, smartroom, overhead, laptop, prefix, courses_to_preferences.prefid FROM courses LEFT JOIN courses_to_preferences ON courses_to_preferences.courseid = courses.id ORDER BY classType";
			Statement stmt = conn.createStatement();
			ResultSet rs = stmt.executeQuery(sql);
			...
			while(rs.next()){
				...
				Course c = new  Course(name, courseNum, wtus, scus, classType, maxEnrollment, 1, lab, overhead, smartroom, laptop, prefix, dfc);
