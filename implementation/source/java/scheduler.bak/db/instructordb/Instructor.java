...
public class Instructor implements Comparable<Instructor>, Serializable
{
   ...
   public static final Instructor STAFF = 
	...
	/**
	 * Returns a new, fresh copy of a given instructor. The important thing here
	 * is that the instructor's availability is not connected with the one being
	 * copied. 
	 * 
    ...
    * of time on a given day. If there is a preference of 0 found in this length
    * of Time, 0 is returned immediately. 
    *
    * @param d The day to check
    * @param s The start time
    * @param length The number of half hours away from "s" the end time should 
    *               be
    *
    * @return the average desire for the given time span
    */
   public double getAvgPrefForTimeRange (int d, Time s, int length)
   {
      ...
      for (int i = 0; i < length; i ++, tempS.addHalf())
      {
         ...
         System.err.println ("Desire on " + d + " for " + tempS + ": " + desire);
         /*
      ...
      System.err.println ("Returning: " + (total / length));
      return (total / length);
   }
