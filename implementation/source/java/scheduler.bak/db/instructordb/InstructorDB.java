...
public class InstructorDB extends Observable {
   ...
   (
      Instructor i, 
      HashMap<Integer, LinkedHashMap<scheduler.db.Time, TimePreference>> tPrefs
   )
   {
      /*
       * Have to change the instructor in the "localData" vector, so I need to 
   ...
   public Vector<Vector> getTimeTable() {
        ...
        for (int i = 0; i < ROWS; i++) {
           scheduler.db.Time start = getStartTime(i);
           scheduler.db.Time end = getEndTime(i);
     ...
     */
    private scheduler.db.Time getStartTime(int number ) {
       int start = 7;
       int hour = (number / 2) + 7; 
       int minute = (number % 2) * 30; 
        
       return new scheduler.db.Time(hour, minute);
    }
     ...
     * @return The end time.
     */
    private scheduler.db.Time getEndTime(int number) {
       int start = 7;
       int hour = (number / 2) + 7; 
       int minute = (number % 2) * 30; 
       if ((minute + 30) == 60) {
           return new scheduler.db.Time(hour + 1, 0);
       }
       else {
           return new scheduler.db.Time(hour, minute + 30);
       }
    }
	...
	public void SetPreferredCourses(Instructor instructor,
			java.util.Vector<scheduler.db.coursedb.Course> course) {
