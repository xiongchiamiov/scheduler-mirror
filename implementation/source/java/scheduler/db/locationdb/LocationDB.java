...
import scheduler.*;
import scheduler.db.SQLDB;
import scheduler.db.coursedb.Course;
import scheduler.db.coursedb.CourseDB;
import scheduler.db.coursedb.CourseDB.CourseDoesNotExistException;
...
public class LocationDB extends Observable {
      ...
      while (iterator.hasNext()) {
         ...
         for (int i = 0; i < NUMDAYSINWEEK; i++ ) {
            ...
            for (int j = 0; j < ROWS; j++) {
               scheduler.db.Time start = getStartTime(j);
               scheduler.db.Time end = getEndTime(j);
               try {
    ...
    *  </pre>
	 *
	 **/
   public scheduler.db.Time getStartTime(int number ) {
    ...
    *  </pre>
	 *
	 **/
   public scheduler.db.Time getEndTime(int number) {
       if (number >= 30 || number < 0) {
           throw new NumberInvalidException();
       }
       int start = 7;
