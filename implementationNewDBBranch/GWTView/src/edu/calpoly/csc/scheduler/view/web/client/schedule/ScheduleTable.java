...
public class ScheduleTable extends FlexTable {
	...
	public static String times[] = { "7:00am", "7:30am", "8:00am", "8:30am",
	...
	void placePanels() {
		...
		for (row = 1; row <= numberOfTimeSlots; row++) {
			for (col = 1; col <= numberOfDays; col++) {
				...
				schdCell.setRow(row);
				// schdCell.setCol(col);
				getFlexCellFormatter().removeStyleName(row, col,
		...
		for (int dayNum : schdItemDays) {
			...
			if (!filteredDays.contains(dayNum)) {
				...
				((ScheduleCell) getWidget(rowRangeStart, dayCol))
						.setScheduleItem(schdItem, getFlexCellFormatter()
								.getElement(rowRangeStart, dayCol)
								.getOffsetHeight());
