...
//		System.out.println("editCourse saving model course " + course.getCatalogNum() + " lectureid " + course.getLectureID());
	...
	@Override
	public Integer openNewDocument(String newOriginalName) {
		System.out.println("GreetingServiceImpl.openNewDocument(\"" + newOriginalName + "\")");

		assert(!model.getSchedules().containsKey(newOriginalName));

		model.openNewSchedule(newOriginalName);
		int newOriginalID = model.getScheduleID();
		assert(newOriginalID > 0);

		model.openNewSchedule("~" + newOriginalName);
		int newWorkingCopyID = model.getScheduleID();
		assert(newWorkingCopyID > 0);
		model.getSchedule().setNote("w. o:" + newOriginalID);
		model.saveSchedule(model.getSchedule());		
		
		model.openExistingSchedule(newOriginalID);
		model.getSchedule().setNote("o. w:" + newWorkingCopyID);
		model.saveSchedule(model.getSchedule());
		assert(model.getScheduleID() == newOriginalID);
		
		return newWorkingCopyID;
	}

	@Override
	public String openExistingDocument(int scheduleID) {
		System.out.println("GreetingServiceImpl.openExistingDocument(" + scheduleID + ")");
		
		model.openExistingSchedule(scheduleID);
		assert(model.getSchedule().getNote().startsWith("o"));
		int workingCopyID = Integer.parseInt(model.getSchedule().getNote().substring(5));

		model.openExistingSchedule(workingCopyID);
		assert(model.getSchedule().getNote().startsWith("w"));
		assert(Integer.parseInt(model.getSchedule().getNote().substring(5)) == scheduleID);

		return model.getSchedule().getName();
	}

	@Override
	public void saveDocument(int workingCopyID) {
		assert(model.getSchedule().getNote().startsWith("w"));
		String workingCopyScheduleName = model.getSchedule().getName();
		assert(workingCopyScheduleName.startsWith("~"));
		String originalScheduleName = workingCopyScheduleName.substring(1);
		
		int originalCopyID = Integer.parseInt(model.getSchedule().getNote().substring(5));

		model.deleteSchedule(originalCopyID);
		model.copySchedule(workingCopyID, originalScheduleName);
		
		assert(model.getScheduleID() == workingCopyID);
	}

	@Override
	public Integer saveDocumentAs(String newOriginalName, boolean allowOverwrite) {
		// TODO: enforce allowOverwrite

		assert(!model.getSchedules().containsKey(newOriginalName));

		model.copySchedule(model.getScheduleID(), newOriginalName);
		int newOriginalID = model.getScheduleID();
		...
		model.openExistingSchedule(newWorkingCopyID);
		
		return newWorkingCopyID;
	}
	
	public Integer revertDocument() {
		int originalID;
		
		{
			int oldWorkingCopyID = model.getScheduleID();
			originalID = Integer.parseInt(model.getSchedule().getName().substring(5));
			model.deleteSchedule(oldWorkingCopyID);
		}
		
		model.openExistingSchedule(originalID);
		String originalName = model.getSchedule().getName();

		model.openNewSchedule("~" + originalName);
		model.getSchedule().setNote("w. o:" + originalID);
		model.saveSchedule(model.getSchedule());		
		int newWorkingCopyID = model.getScheduleID();
		...
		model.getSchedule().setNote("o. w:" + newWorkingCopyID);
		model.saveSchedule(model.getSchedule());
		assert(model.getScheduleID() == originalID);
