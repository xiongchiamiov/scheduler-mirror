 ...
 * between the two lists. Uses modified code from Fred Sauer's example at
 * http://
 * allen-sauer.com/com.allen_sauer.gwt.dnd.demo.DragDropDemo/DragDropDemo.
 * html#DualListExample
 * 
...
public class DualListBox extends AbsolutePanel {
	...
	// Adds a selected course in the "Available Courses" list to the
	 ...
	 * 
	 * @param visibleItems
	 *            The number items viewable in the list boxes.
	 * @param width
	 *            The width of the list boxes.
	 * @param totalItems
	 *            The total number of items the list boxes will hold.
	 * @param schedule
	 *            The ScheduleViewWidget in which this panel resides.
	 */
	...
	/**
	 * Registers drop controllers located on a weekly schedule view's time
	 * slots.
	 * 
	 * @param dropController
	 *            A timeslot's DropController
	 */
	...
	/**
	 * Registers the two lists' DropControllers with their DragController.
	 * Called after the DragController has cleared its record of
	 * DropControllers.
	 */
	...
	/**
	 * Move all courses in the "Available Courses" list to the
	 * "Courses to be Scheduled" list.
	 */
	private void moveAllToIncluded() {
		for (Widget widget : left.widgetList()) {
			// If the course is already on the schedule, or in the
			// "Courses to be Scheduled" list then don't add it.
			System.out
					.println(((CourseListItem) widget).getCourse().toString());
	...
	private void moveSelectedToIncluded() {
		...
		for (Widget item : selectedItems) {
			...
			// If the course is already on the schedule, or in the
			// "Courses to be Scheduled" list, don't add it.
